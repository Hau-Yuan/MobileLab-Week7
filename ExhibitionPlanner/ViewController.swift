...
let ModelAssets: [(name: String, nodeName: String)] = [
    ("InstallationType1", "installationType1"),
    ("InstallationType2", "installationType2"),
    ("Projection", "projection"),
    ("MonitorBased", "monitorBased")
]
...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        // Setup pinch gesture for screen.
        let pinchGesture = UIPinchGestureRecognizer(target: self, action: #selector(handlePinch))
        sceneView.addGestureRecognizer(pinchGesture)
        
        // Setup pan gesture for screen.
    ...
    func handlePinch(_ recognizer: UIPinchGestureRecognizer) {
        ...
        currentScale += recognizer.velocity * 0.01
        if let node = lastTouchedNode {
            node.scale = SCNVector3Make(Float(currentScale), Float(currentScale), Float(currentScale))
        }
    }
    ...
    func handlePan(_ recognizer: UIPanGestureRecognizer) {
        ...
        currentAngle = translation.x * 0.01
        if let node = lastTouchedNode {
            node.eulerAngles = SCNVector3(0, Float(currentAngle!), 0)
        }
    }
